...
\end{figure}
	...
	Platformą programistyczną systemu informatycznego został .NET 5, który pozwala na tworzenie aplikacji wieloplatformowych. Dzięki wykorzystaniu następcy .NET Core, aplikacja może zostać uruchomiona zarówno na urządzeniach z systemem Microsoft jak i dystrybucjach systemu Linux co pozwala na elastyczność względem wyboru środowiska w którym będzie pracować aplikacja.
	
	Do stworzenia interfejsu REST API została wykorzystana biblioteka ASP.NET Core, która pozwala na stworzenie metod, które będą konsumowane przez warstwę prezentacji. ASP.NET Core wspiera wykorzystanie kontenerów wstrzykiwania zależności, obsługę autoryzacji przy pomocy żetonu JWT jak i komunikację z wykorzystaniem protokołu HTTPS.
	...
	Jako bazę danych wykorzystano MongoDB. MongoDB jest dokumentową bazą danych pozwalająca na przechowywanie danych w formacie JSON, który jest czytelny dla człowieka. Baza danych należy do nierelacyjnych baz danych NoSQL, której głównymi zaletami są: elastyczny schemat bazy, łatwe skalowanie wszerz oraz możliwość tworzenia zaawansowanych zapytań i raportów.
	...
	Do stworzenia interfejsu użytkownika została wykorzystana platforma programistyczna Angular, która pozwala na tworzenie komponentów wielokrotnego użytku oraz hermetyzację logiki w postaci serwisów aplikacyjnych. Cechą charakterystyczną platformy jest natywne wykorzystanie języka TypeScript oraz możliwość tworzenia aplikacji typu SPA (Single page application).
